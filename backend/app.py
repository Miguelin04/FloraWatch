...
from src.data_sources.openweather_api import OpenWeatherClient
...
def health_check():
    ...
    return jsonify({
        ...
        'services': {
            ...
            'openweather_api': weather_client.is_available(),
...
def get_weather_data():
    ...
    try:
        ...
        weather_data = weather_client.get_weather_for_flowering_analysis(lat, lon)
        
        if not weather_data:
            return jsonify({'error': 'No se pudieron obtener datos meteorológicos'}), 404
...
@app.route('/api/integrated-analysis')
def get_integrated_analysis():
    """Obtener análisis integrado combinando NASA + OpenWeather"""
    try:
        lat = request.args.get('lat', type=float)
        lon = request.args.get('lon', type=float)
        start_date = request.args.get('start_date', 
        ...
        logger.info(f"Análisis integrado para {lat}, {lon}")
        
        # Verificar cache
        cache_key = f"integrated_{lat}_{lon}_{start_date}_{end_date}"
        cached_result = cache_manager.get(cache_key)
        
        if cached_result:
            return jsonify(cached_result)
        
        # Obtener datos de NASA (satelitales)
        nasa_data = nasa_client.get_integrated_earth_data(lat, lon, start_date, end_date)
        
        # Obtener datos meteorológicos
        weather_data = weather_client.get_weather_for_flowering_analysis(lat, lon)
        
        # Procesar datos satelitales
        if nasa_data.get('vegetation_data'):
            processed_data = data_processor.process_time_series(nasa_data['vegetation_data'])
        ...
        integrated_result = {
            ...
            'flowering_events': flowering_events,
            'events_detected': len(flowering_events),
            'analysis_quality': _calculate_analysis_quality(nasa_data, weather_data),
            'recommendations': _generate_recommendations(flowering_events, weather_data),
            'metadata': {
                'data_sources': nasa_data.get('data_sources', []) + ['OpenWeatherMap'],
                'processed_at': datetime.utcnow().isoformat(),
        ...
        # Guardar en cache
        cache_manager.set(cache_key, integrated_result, expiry_hours=6)
...
def get_nasa_api_status():
    """Verificar estado de todas las APIs de NASA"""
    try:
        status = nasa_client.get_api_status()
        return jsonify(status)
    except Exception as e:
        logger.error(f"Error verificando estado NASA: {str(e)}")
        return jsonify({'error': 'Error verificando APIs'}), 500

def _calculate_analysis_quality(nasa_data: Dict, weather_data: Dict) -> Dict:
    """Calcular calidad del análisis basado en datos disponibles"""
    quality_score = 0
    factors = []
    
    # Calidad de datos satelitales
    if nasa_data.get('vegetation_data'):
        ts = nasa_data['vegetation_data'].get('time_series', {})
        if ts.get('values'):
            satellite_quality = len([f for f in ts.get('quality_flags', []) if f == 'good']) / len(ts.get('values', [1]))
            quality_score += satellite_quality * 0.6
            factors.append(f"Datos satelitales: {satellite_quality:.1%}")
    
    # Calidad de datos meteorológicos
    if weather_data.get('current_weather'):
        quality_score += 0.3
        factors.append("Datos meteorológicos actuales: 100%")
    
    if weather_data.get('forecast'):
        quality_score += 0.1
        factors.append("Pronóstico meteorológico: 100%")
...
def _generate_recommendations(flowering_events: List[Dict], weather_data: Dict) -> List[str]:
    """Generar recomendaciones basadas en el análisis"""
    recommendations = []
    
    if flowering_events:
        recommendations.append(f"Se detectaron {len(flowering_events)} eventos de floración")
        
        # Recomendaciones basadas en eventos
        for event in flowering_events[-2:]:  # Últimos 2 eventos
            if event.get('confidence', 0) > 0.8:
                recommendations.append(f"Evento de alta confianza detectado el {event.get('peak_date', 'fecha desconocida')}")
    
    # Recomendaciones meteorológicas
    if weather_data.get('flowering_conditions'):
        conditions = weather_data['flowering_conditions']
        
        if conditions.get('flowering_probability', 0) > 0.7:
            recommendations.append("Condiciones meteorológicas favorables para floración")
        elif conditions.get('temperature_favorability', 0) < 0.5:
            recommendations.append("Temperaturas actuales no óptimas para floración")
        
        if conditions.get('precipitation_risk', 0) > 0.6:
            recommendations.append("Alto riesgo de precipitación - monitorear efectos en floración")
    
    if not recommendations:
        recommendations.append("Continuar monitoreo para detectar cambios estacionales")
